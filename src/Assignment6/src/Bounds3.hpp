...
inline bool Bounds3::IntersectP(const Ray& ray, const Vector3f& invDir) const
{
    ...
    Vector3f tEnter = (pMin - ray.origin) * invDir;
    Vector3f tExit  = (pMax - ray.origin) * invDir;
    ...
    float tIn, tOut;
    tIn = std::fmax(std::fmax(tEnter.x, tEnter.y),tEnter.z);
    tOut = std::fmin(std::fmin(tExit.x, tExit.y), tExit.z);
    if (tIn < tOut && tOut > 0)
        return true;
    else
        return false;
