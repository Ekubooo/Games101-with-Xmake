...
void rst::rasterizer::draw(std::vector<Triangle *> &TriangleList) {
    ...
    for (const auto& t:TriangleList)
    {
        ...
        MSAA_triangle(newtri, viewspace_pos);
    }
}
...
static Eigen::Vector2f interpolate
    (float alpha, float beta, float gamma, const Eigen::Vector2f& vert1, 
...
{
    auto v = t.toVector4();
    float xMax = INT_MIN, xMin = INT_MAX, yMax =INT_MIN, yMin = INT_MAX, zMin = INT_MAX;
    for (auto &i : v)
    {
    ...
    for (float x = xMin; x <= xMax; x++)
    {
        ...
        {   
            ...
            {   
                ...
                float Z = alpha / v[0].w() + beta / v[1].w() + gamma / v[2].w();
                ...
                if (-zp < depth_buf[ind])
                {   
                    Eigen::Vector3f itp_color 
                        = interpolate(alpha,beta,gamma,t.color[0],t.color[1],t.color[2],Z);
                    Eigen::Vector3f itp_normal 
                        = interpolate(alpha,beta,gamma,t.normal[0],t.normal[1],t.normal[2],Z);
                    Eigen::Vector2f itp_texcoords 
                        = interpolate(alpha,beta,gamma,t.tex_coords[0],t.tex_coords[1],t.tex_coords[2],Z);;
                    Eigen::Vector3f itp_shadingcoords
                        = interpolate(alpha,beta,gamma,view_pos[0],view_pos[1],view_pos[2],Z);;
                    fragment_shader_payload payload
                        (itp_color, itp_normal.normalized(), itp_texcoords, texture ? &*texture : nullptr);
...
// ---------------------------------------------------------------------------
void rst::rasterizer::MSAA_triangle
    (const Triangle& t, const std::array<Eigen::Vector3f, 3>& view_pos) 
 {
    ...
    for (int i = 0; i < 4; i++)
    {
        ...
        {   
            ...
            {   
                ...
                depthSample[index *4 + i] = -zp;
                colorSample[index *4 + i] = pixel_color / 4.0f;
            }
        }
        finalColor += colorSample[index *4 + i];
        zMin = std::min(zMin,depthSample[index *4 + i]);
    }
    set_pixel({x,y},finalColor);
...
/*
    zp *= Z;
    itp_color     = alpha * t.color[0] + beta * t.color[1] + gamma * t.color[2];
    itp_normal    = alpha * t.normal[0] + beta * t.normal[1] + gamma * t.normal[2];
    itp_texcoords = alpha * t.tex_coords[0] + beta * t.tex_coords[1] + gamma * t.tex_coords[2];
    itp_color *= Z;
    itp_normal*= Z;
    itp_texcoords*= Z;
*/
