...
inline bool Bounds3::IntersectP(const Ray& ray, const Vector3f& invDir,const std::array<int, 3>& dirIsNeg) const
{
    ...
    Vector3f rayOrg = ray.origin;

    Vector3f tEnter = (pMin - rayOrg) * invDir;
    Vector3f tExit  = (pMax - rayOrg) * invDir;
    if(rayDir.x <= 0)
    ...
    float tIn, tOut;
    tIn = std::max(std::max(tEnter.x, tEnter.y),tEnter.z);
    tOut = std::min(std::min(tExit.x, tExit.y), tExit.z);
    if (tIn <= tOut && tOut >= 0)
        return true;
    else
        return false;
